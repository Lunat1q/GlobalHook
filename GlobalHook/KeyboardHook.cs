using System;
using System.Runtime.InteropServices;
using GlobalHook.Event;

namespace GlobalHook
{
    /// <summary>
    /// Captures global keyboard events
    /// </summary>
    public class KeyboardHook : GlobalHook
    {

        #region Events

        public IEventDispatcher EventDispatcher;

        #endregion

        #region Constructor

        public KeyboardHook()
        {
            EventDispatcher = new EventDispatcher();
            _hookType = WH_KEYBOARD_LL;

        }

        #endregion

        #region Methods

        protected override int HookCallbackProcedure(int nCode, int wParam, IntPtr lParam)
        {

            bool handled = false;

            if (nCode > -1)
            {

                KeyboardHookStruct keyboardHookStruct =
                    (KeyboardHookStruct)Marshal.PtrToStructure(lParam, typeof(KeyboardHookStruct));

                // Is Control being held down?
                bool control = ((GetKeyState(VK_LCONTROL) & 0x80) != 0) ||
                               ((GetKeyState(VK_RCONTROL) & 0x80) != 0);

                // Is Shift being held down?
                bool shift = ((GetKeyState(VK_LSHIFT) & 0x80) != 0) ||
                             ((GetKeyState(VK_RSHIFT) & 0x80) != 0);

                // Is Alt being held down?
                bool alt = ((GetKeyState(VK_LALT) & 0x80) != 0) ||
                           ((GetKeyState(VK_RALT) & 0x80) != 0);

                // Is CapsLock on?
                bool capslock = (GetKeyState(VK_CAPITAL) != 0);

                // Create event using keycode and control/shift/alt values found above
                KeyEventArguments e = new KeyEventArguments()
                {
                    KeyPressed = keyboardHookStruct.vkCode,
                    Modifiers = (control ? KeyModifier.Ctrl : 0) |
                        (shift ? KeyModifier.Shift : 0) |
                        (alt ? KeyModifier.Alt : 0)
                };


                // Handle KeyDown and KeyUp events
                switch (wParam)
                {

                    case WM_KEYDOWN:
                    case WM_SYSKEYDOWN:
                        var ev = new KeyDownEvent
                        {
                            Keys = e
                        };
                        EventDispatcher.Send(ev);
                        handled = ev.Handled;
                        break;
                    case WM_KEYUP:
                    case WM_SYSKEYUP:
                        var ev2 = new KeyUpEvent
                        {
                            Keys = e
                        };
                        EventDispatcher.Send(ev2);
                        handled = ev2.Handled;
                        break;

                }

                // Handle KeyPress event
                if (wParam == WM_KEYDOWN &&
                   !handled)
                {

                    byte[] keyState = new byte[256];
                    byte[] inBuffer = new byte[2];
                    GetKeyboardState(keyState);

                    if (ToAscii(keyboardHookStruct.vkCode,
                              keyboardHookStruct.scanCode,
                              keyState,
                              inBuffer,
                              keyboardHookStruct.flags) == 1)
                    {

                        char key = (char)inBuffer[0];
                        if ((capslock ^ shift) && Char.IsLetter(key))
                            key = Char.ToUpper(key);
                        KeyPressEvent e2 = new KeyPressEvent()
                        {
                            Key = key
                        };
                        EventDispatcher.Send(e2);
                        handled = e2.Handled;

                    }

                }

            }

            return handled ? 1 : CallNextHookEx(_handleToHook, nCode, wParam, lParam);
        }

        #endregion

    }
}

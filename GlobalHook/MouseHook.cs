...
namespace GlobalHook
{
    ...
    public class MouseHook : GlobalHook
    {
        ...
        public MouseHook(bool lowLevel = true)
        {
            EventDispatcher = new EventDispatcher();
            _hookType = lowLevel ? WH_MOUSE_LL : WH_MOUSE;
            ...
            if (nCode > -1)
            {
                ...
                switch (eventType)
                {
                    case MouseEventType.MouseDown:
                        EventDispatcher.Send(new MouseDownEvent { Button = button });
                        break;
                    case MouseEventType.MouseUp:
                        EventDispatcher.Send(new MouseDownUp { Button = button });
                        break;
                    case MouseEventType.DoubleClick:
                        EventDispatcher.Send(new MouseDownDoubleClick { Button = button });
                        break;
